...
void busca(char *nomeAux, Pessoa *pAtual, Pessoa *pInicio, int *qt);
...
int main() {
	...
	*/
	...
	//DICA DO TORCHELSEN: PRIMEIRO VALOR DO BUFFER SER O TAMANHO DELE, E DEPOIS ALOCAR MEMÓRIA SEMPRE DE ACORDO COM O TAMANHO
	pBuffer = malloc(sizeof(int)*3 + sizeof(char)*20);
	qt = (int *)pBuffer;
	*qt = 0; //seta como 0
	opcao = (int *)(qt + sizeof(int)); //opção aponta pra qt + 4	
	...
	do {
		...
		if(*opcao == 1) {
			*qt = (*qt + 1); //se a opção foi de inserir, adiciona mais um na quantidade
			...
			x = (int *)(opcao + sizeof(int));
			*x = 0;
			nomeAux = (char *)(x + (sizeof(int)*3));//laranja com batata pbuffer n pode somar com sizeof int
		...
		if(*opcao == 3) {
			pInicio = (Pessoa *) ((sizeof(int)*3) + (sizeof(char)*20) + (sizeof(Pessoa) * (*qt)));
			printf("Quem você deseja buscar? ");
			scanf("%s", nomeAux);
			busca(nomeAux, pAtual, pInicio, qt);
		}
	} while(*opcao != 0);
